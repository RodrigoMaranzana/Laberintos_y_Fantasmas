...
static void _dibujado_jugador_sin_vida(SDL_Texture *textura);
...
static void _dibujado_jugador_sin_vida(SDL_Texture *textura)
{
    SDL_SetTextureColorMod(textura, 255, 0, 0);
    SDL_SetTextureBlendMode(textura, SDL_BLENDMODE_ADD);
}
...
static void _dibujado_entidad(SDL_Renderer *renderer, SDL_Texture **imagenes, tEntidad *entidad)
{
    ...
    } else if (entidad->tipo == ENTIDAD_JUGADOR && entidad->estado == ENTIDAD_SIN_VIDA) {
        efecto = _dibujado_jugador_sin_vida;
